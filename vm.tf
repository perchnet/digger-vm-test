module "vm_block_storage" {
  source = "github.com/b-/terraform-bpg-proxmox//modules/block_storage?ref=e0f7598"
  node    = local.pve_node
  storage = "zssd"
  size    = 10
  name    = "block-storage"
...
resource "terraform_data" "tskey_stable" {
  ...
  lifecycle {
    ...
    replace_triggered_by = [terraform_data.tskey_stable_replacement_hook]
  }
}
  ...
  cloudinit = {
    ...
    user_data        = <<-EOF
      ...
      runcmd:
        ...
        - [ 'tailscale', 'up', '--authkey', ${terraform_data.tskey_stable.output},
            '--accept-routes',
            '--accept-dns',
            '--ssh',
